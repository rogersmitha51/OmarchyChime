...
//
// The host (Service.qml) supplies the three external gates:
...
Item {
  ...
  // safe, not ready, overlap unknown (blocked).
  ...
  // Gate changes that only govern automatic playback (desktop activation,
  // overlap, desktop readiness) must not cut a preview short.
  ...
  // Automatic playback must stop the moment any gate closes: mute, desktop
  // deactivation, overlap, or desktop readiness loss. Safety loss and mute
  // stop previews too; the desktop/overlap gates leave previews alone.
