// Omarchy Chime — desktop event sounds.
//
// Independent service plugin (nick.chime). This file is a small composition
// root only: it wires the ChimeController (audio/settings) and DesktopEvents
// (window/workspace adapter) slices together, resolves the original
// DND/lock/idle services for conservative safety gating, and exposes the
// `chime` IPC surface. It owns no notification daemon, popup, history, or
// DND state — the built-in omarchy.notifications service keeps all of that.
...
Item {
  ...
  IpcHandler {
    ...
    function help(): string {
      return "chime commands: status, mute, unmute, volume <0-1>, desktop on|off, preview <eventName>"
    }
  }
